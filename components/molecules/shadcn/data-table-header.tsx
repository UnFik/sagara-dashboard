      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            size="sm"
            className={cn(
              `-ml-3 h-8 data-[state=open]:bg-gray-300 text-xs ${className}`
            )}
          >
            <span className="max-w-72 break-words text-wrap text-left line-clamp-2">
              {title}
            </span>
            {column.getIsSorted() === "desc" ? (
              ...
              <Icons.chevronsUpDown className="ml-2 max-h-4 max-w-4" />
            )}
          </Button>
        </DropdownMenuTrigger>
